...
#ifdef ARDUINO_AVR_SIMULATOR_TYPE2
// Only 1 signal LED, the right hand LED in the power/data RJ45 connector
const int LED = 4;
#endif
...
// the remapper supports these (and allows a bell to be configured as a switch!).
	...
	for ( i = 0; i < numChannels; i++ ) {
		...
		case WAIT_FOR_INPUT:
			...
			if ( channelSensorValue == LOW && channelLastValue[i] == HIGH ) {
				...
				if ( debugThisChannel( i ) && isDebugFlagSet( DEBUG_PULSE_TIMER | DEBUG_SHOW_MISFIRES ) ) {
		...
		case WAIT_FOR_DEBOUNCE:
			...
			else if ( timeNowMicros >= channelDebounceEndTime[i] )
			{
				...
				if ( debugThisChannel( i ) && isDebugFlagSet( DEBUG_SHOW_DEBOUNCE ) ) {
					Serial.print( i + 1 ); // The channel number, as 1-16
					Serial.print(F(" "));
					Serial.print( bellStrikeChar[i]);
					Serial.print(F(" "));
